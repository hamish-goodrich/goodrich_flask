...
pond_view = Blueprint('pond_view', __name__)

@pond_view.route('/pond_monitoring', methods=['GET', 'POST'])
@login_required
def pond_monitoring():
...
@pond_view.route("/view_log/<int:log_id>")
def view_log(log_id):
...
@pond_view.route('/edit_log/<int:log_id>', methods=['GET', 'POST'])
def edit_log(log_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect(url_for('pond_view.view_log', log_id=log.id, user=current_user))

    return render_template('edit_log.html', log=log, user=current_user)


@pond_view.route('/delete-log', methods=['POST'])
def delete_log():
    ...
    db.session.commit()
    return redirect(url_for('pond_view.pond_monitoring'))  # Redirect back to the home page

@pond_view.route('/createlog', methods=['GET', 'POST'])
@login_required
def createlog():
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect(url_for('pond_view.pond_monitoring'))

    return render_template("createlog.html", user=current_user)

@pond_view.route('/download_csv')
def download_csv():
...
@login_required
def view_pond():
    return render_template("view_pond.html", user=current_user)
